...
<script type="text/javascript">
  ...
  $('.product-link').on('click', function () {
	  ...
	  const rawProdNoAttr = $this.attr('data-prodno');
	  const prodNo = rawProdNoAttr ? rawProdNoAttr.toString().trim() : "";

	  const actionAttr = $this.attr('data-action');

	  console.log("[디버깅] 클릭한 요소 outerHTML:", this.outerHTML);
	  console.log("[디버깅] 클릭한 상품의 action:", actionAttr);

	  if (!prodNo) {
	    alert("❌ 상품번호가 비어 있습니다. 이동 중단");
	    return;
	  }

	  const finalUrl = "/product/getProduct?prodNo=" + encodeURIComponent(prodNo);
	  console.log("[디버깅] 최종 이동할 URL:", finalUrl);
